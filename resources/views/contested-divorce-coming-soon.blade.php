...
<body class="coming-soon-page">
    ...
    <dialog class="inquiry-modal" data-inquiry-modal aria-labelledby="inquiry-modal-title">
        ...
        <form class="inquiry-form inquiry-modal-form" data-inquiry-form data-send-url="{{ route('inquiry.tac.send') }}" data-verify-url="{{ route('inquiry.tac.verify') }}" data-submit-url="{{ route('inquiry.submit') }}" novalidate>
            ...
            <div data-inquiry-stage="contact"><label><span>Full name</span><input type="text" name="full_name" autocomplete="name" maxlength="150" required></label><label><span>Email address</span><input type="email" name="email" autocomplete="email" maxlength="254" required></label><button class="button" type="button" data-inquiry-send>Send verification code <span>→</span></button></div>
            <div data-inquiry-stage="verify" hidden><p class="inquiry-stage-note">Enter the code sent to <strong data-inquiry-email></strong>.</p><div class="tac-boxes" data-inquiry-tac aria-label="Six-digit verification code">@for ($digit = 1; $digit <= 6; $digit++)<input type="text" inputmode="numeric" autocomplete="{{ $digit === 1 ? 'one-time-code' : 'off' }}" pattern="[0-9]" maxlength="1" required aria-label="Verification code digit {{ $digit }}" data-inquiry-digit>@endfor</div><div class="inquiry-button-row"><button class="button button-outline" type="button" data-inquiry-change>Change email</button><button class="button" type="button" data-inquiry-verify>Verify email <span>→</span></button></div><button class="quiet-button inquiry-resend" type="button" data-inquiry-resend>Send another code</button></div>
            <div data-inquiry-stage="details" hidden><div class="verified-email"><span aria-hidden="true">✓</span><div><strong>Email verified</strong><small data-inquiry-verified-email></small></div></div><input type="hidden" name="topic" value="other"><label><span>Your enquiry</span><textarea name="message" rows="5" minlength="10" maxlength="2000" required placeholder="Tell us about your situation and what support you need."></textarea><small>10–2,000 characters</small></label><label class="inquiry-consent"><input type="checkbox" name="privacy_consent" value="1" required><span>I consent to LegalDIY storing these details to review and respond to my enquiry.</span></label><button class="button" type="submit">Send enquiry <span>→</span></button></div>
